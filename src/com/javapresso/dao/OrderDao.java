...
//			String orderSql = "INSERT INTO orders (order_id, order_date, customer_id, menu_name, request, is_ice, use_coupon) "
//					+ "VALUES (order_seq.nextval, SYSDATE, ?, ?, ?, ?, ?) returning order_id into  ?";
...
// 기존 회원인가 확인
	...
	public boolean insertMember(String customerId) throws SQLException {
		String sql = "INSERT INTO members (customer_id, stamp, coupon) VALUES (?, 1, 0)";
		try (Connection con = dataSource.getConnection()) {
			PreparedStatement ps = con.prepareStatement(sql);
			ps.setString(1, customerId);
...
// 3. 주문내역 확인하기
	public List<OrderDto> getOrderList() {
		...
		try (Connection con = dataSource.getConnection()) {
			String sql = "SELECT order_id AS orderId, " + "customer_id AS customerId, " + "menu_name AS menuName, "
					+ "order_date AS orderDate " + "FROM orders " + "WHERE TRUNC(order_date) = TRUNC(SYSDATE) "
